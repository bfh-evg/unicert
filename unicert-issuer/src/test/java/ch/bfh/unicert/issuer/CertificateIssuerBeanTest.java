...
public class CertificateIssuerBeanTest {
	...
	public void testCreateCertificate1() throws Exception {
		...
		ByteArrayToBigInteger byteArrayConverter
				= ByteArrayToBigInteger.getInstance(hashMethod.getHashAlgorithm().getByteLength(), 1);
		FiatShamirSigmaChallengeGenerator scg = FiatShamirSigmaChallengeGenerator.getInstance(Z_q, message,
				convertMethod, hashMethod, byteArrayConverter);
		PlainPreimageProofSystem pips = PlainPreimageProofSystem.getInstance(scg, func);

		Triple proof = pips.generate(x, y);

		DiscreteLogSetup dls = new DiscreteLogSetup(2048, g.convertToBigInteger(), p, q, y.convertToBigInteger(),
				proof.getFirst().convertToBigInteger(), proof.getSecond().convertToBigInteger(),
				proof.getThird().convertToBigInteger());
	...
	public void testCreateCertificate2() throws Exception {
		...
		RSASignatureScheme rss = RSASignatureScheme.getInstance(StringMonoid.getInstance(Alphabet.UNICODE_BMP),
				ZModPrimePair.getInstance(p, q), convertMethod, hashMethod);
...
//        Element message = rss.getZMod().getElement(new BigInteger(1, messageHashed.getBytes()));
		...
		RsaSetup rs = new RsaSetup(2 * size, n, keys.getAt(1).convertToBigInteger(), signature.convertToBigInteger());
