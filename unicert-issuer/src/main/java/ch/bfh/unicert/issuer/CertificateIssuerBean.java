...
public class CertificateIssuerBean implements CertificateIssuer {
	...
	public static final HashMethod HASH_METHOD = HashMethod.getInstance(HashAlgorithm.SHA256);
	public static final ConvertMethod CONVERT_METHOD = ConvertMethod.getInstance(
	...
	public Certificate createCertificate(CryptographicSetup cs, IdentityData idData, String applicationIdentifier,
		...
		if (cs instanceof RsaSetup) {
			...
			RSASignatureScheme rsa = RSASignatureScheme.getInstance(StringMonoid.getInstance(Alphabet.UNICODE_BMP),
					rsaGroup, CONVERT_METHOD, HASH_METHOD);
			...
			ByteArrayToBigInteger byteArrayConverter
					= ByteArrayToBigInteger.getInstance(HASH_METHOD.getHashAlgorithm().getByteLength(), 1);
			FiatShamirSigmaChallengeGenerator scg = FiatShamirSigmaChallengeGenerator.getInstance(setup.getZ_q(), s,
					CONVERT_METHOD, HASH_METHOD, byteArrayConverter);
			PreimageProofSystem pips = PlainPreimageProofSystem.getInstance(scg, func);
			...
			pk = createDSAPublicKey(setup.getPublicKey().convertToBigInteger(), setup.getP().getValue(), setup.getZ_q().
					getOrder(), setup.getGenerator().convertToBigInteger());
		...
		if (getConfigurationHelper().getUniBoardServiceURL() != null) {
			postOnUniBoard(cert, getConfigurationHelper().getUniBoardWsdlURL(),
					getConfigurationHelper().getUniBoardServiceURL(), getConfigurationHelper().
