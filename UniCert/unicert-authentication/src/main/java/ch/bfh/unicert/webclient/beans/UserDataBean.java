...
/**
 * This bean reads all relevant for UniCert retrieved from the identity provider during a session. Depending on the
 * context, some of the values can be null. Thus, clients cannot rely on the fact that all fields are always
 * initialized.
 * <p>
 * When used in production, relevant fields are initialized upon calling method readValues().
 *
...
public class UserDataBean implements Serializable {
	...
	if (Boolean.parseBoolean(externalContext.getInitParameter("dev-mode"))) {
	    ...
	    ud = new SwitchAAIUserData("1-2-3-4", null, null, null, "11-222-333", null, "Test name",
		    "test surname", "philemon.vonbergen@bfh.ch", "Law", "Master", null,
    ...
    /**
     * Set the user data if they are construct through another way than the readValue() method
     *
